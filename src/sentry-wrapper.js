...
module.exports = function (RED) {
    ...
    function SentryNode(config) {
        ...
        let node = this;
        let ENVIRONMENT = RED.util.getSetting(node, 'ENVIRONMENT')

        Sentry.init({dsn: config.dsn, environment: config.environment || ENVIRONMENT || 'dev'});

        node.on('input', function (msg) {
            configureSentryScopeFromMessage(msg);
            captureExceptionFromMessage(node, msg);
        });
    }

    function configureSentryScopeFromMessage(msg) {
        if (!isValidObject(msg.sentry)) {
            return;
        }
        const msgConfig = msg.sentry;
        ...
        if (isValidObject(msgConfig.tags) && Object.keys(msgConfig.tags).length > 0) {
            Sentry.configureScope(function (scope) {
                scope.setTags(msgConfig.tags);
            });
        }
    }
    ...
    function captureExceptionFromMessage(node, msg) {
        let errorSent = false;
        try {
            if (isValidErrorObject(msg.error)) {
                usePreviousErrorAsBreadcrumb(msg)
    ...
    function usePreviousErrorAsBreadcrumb(msg) {
        if (!isValidErrorObject(msg._error)) {
            return;
        }
        wrapError(msg._error, err => Sentry.addBreadcrumb({
    ...
    /**
     * convert node red error object to native error and prepare its data to be sent to Sentry
    ...
    function wrapError(err, callback) {
        Sentry.withScope(scope => {
            ...
            if (typeof callback === 'function') {
                ...
                errObject.stack = buildErrorStackTrace(err.message, err.source.id);
                errObject.data = err;
                callback(errObject);
            }
        });
     ...
     **/
    function buildErrorStackTrace(error_message, node_id) {
        ...
        let error_line = '';
        try {
            error_line = node_info.func.split("\n")[line - 1];
        } catch (e) {
        }
    ...
    /**
     * try to get the details of the node using its {node_id}
